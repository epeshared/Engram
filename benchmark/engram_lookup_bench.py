...
class NgramHashMapping:
    ...
    def hash(self, input_ids, *, return_timings: bool = False):
        t0 = time.perf_counter()
        input_ids = self.compressed_tokenizer(input_ids)
        t1 = time.perf_counter()

        out: Dict[int, np.ndarray] = {}
        per_layer_s: Dict[int, float] = {}
        for layer_id in self.layer_ids:
            tl0 = time.perf_counter()
            out[int(layer_id)] = self._get_ngram_hashes(input_ids, layer_id=int(layer_id))
            tl1 = time.perf_counter()
            per_layer_s[int(layer_id)] = (tl1 - tl0)

        if not return_timings:
            return out

        timings = {
            "compressed_tokenizer_s": (t1 - t0),
            "get_ngram_hashes_s": float(sum(per_layer_s.values())),
            "get_ngram_hashes_per_layer_s": per_layer_s,
        }
        return out, timings
...
def main() -> None:
    ...
    hash_s_total = 0.0
    hash_tokenizer_s_total = 0.0
    hash_get_ngram_s_total = 0.0
    hash_get_ngram_per_layer_s_total: Dict[int, float] = defaultdict(float)
    to_tensor_s_total = 0.0
    mhe_s_total = 0.0
    flatten_s_total = 0.0
    emb_s_total = 0.0

    def one_pass(measure: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
        nonlocal hash_s_total, hash_tokenizer_s_total, hash_get_ngram_s_total
        nonlocal hash_get_ngram_per_layer_s_total, to_tensor_s_total
        nonlocal mhe_s_total, flatten_s_total, emb_s_total

        if measure:
            t0 = time.perf_counter()

        if measure:
            hashed_dict, hash_timings = hash_mapping.hash(input_ids_np, return_timings=True)
        else:
            hashed_dict = hash_mapping.hash(input_ids_np)
            hash_timings = None

        hashed_np = hashed_dict[args.layer_id]

        if measure:
            t1 = time.perf_counter()

        hash_input_ids = torch.from_numpy(hashed_np)

        if measure:
            t2 = time.perf_counter()

        emb_out = mhe(hash_input_ids)

        if measure:
            t3 = time.perf_counter()

        embeddings = emb_out.flatten(start_dim=-2)

        if measure:
            t4 = time.perf_counter()
            hash_s_total += (t1 - t0)
            if hash_timings is not None:
                hash_tokenizer_s_total += float(hash_timings["compressed_tokenizer_s"])
                hash_get_ngram_s_total += float(hash_timings["get_ngram_hashes_s"])
                for lid, dt in hash_timings["get_ngram_hashes_per_layer_s"].items():
                    hash_get_ngram_per_layer_s_total[int(lid)] += float(dt)
            to_tensor_s_total += (t2 - t1)
            mhe_s_total += (t3 - t2)
            flatten_s_total += (t4 - t3)
            emb_s_total += (t4 - t2)

        return hash_input_ids, embeddings

    for _ in range(max(0, args.warmup)):
        _ = one_pass(measure=False)

    # os.system("emon -collect-edp > emon.dat &")
    t0 = time.perf_counter()
    for _ in range(max(1, args.runs)):
        hash_ids, emb = one_pass(measure=True)
    t1 = time.perf_counter()
    # os.system("emon -stop")

    dt_ms = (t1 - t0) * 1000.0 / max(1, args.runs)
    hash_ms = (hash_s_total * 1000.0) / max(1, args.runs)
    hash_tokenizer_ms = (hash_tokenizer_s_total * 1000.0) / max(1, args.runs)
    hash_get_ngram_ms = (hash_get_ngram_s_total * 1000.0) / max(1, args.runs)
    to_tensor_ms = (to_tensor_s_total * 1000.0) / max(1, args.runs)
    mhe_ms = (mhe_s_total * 1000.0) / max(1, args.runs)
    flatten_ms = (flatten_s_total * 1000.0) / max(1, args.runs)
    emb_ms = (emb_s_total * 1000.0) / max(1, args.runs)
    ...
    print(
        f"[Time] avg per run = {dt_ms:.3f} ms over runs={args.runs} (warmup={args.warmup})"
    )
    print(
        f"[Time] hash={hash_ms:.3f} ms | np->torch={to_tensor_ms:.3f} ms | "
        f"mhe={mhe_ms:.3f} ms | flatten={flatten_ms:.3f} ms | mhe+flatten={emb_ms:.3f} ms"
    )
    print(
        f"[Time] hash breakdown: compressed_tokenizer={hash_tokenizer_ms:.3f} ms | "
        f"_get_ngram_hashes(all layers)={hash_get_ngram_ms:.3f} ms"
    )

    if len(hash_get_ngram_per_layer_s_total) > 0:
        per_layer_items = sorted(hash_get_ngram_per_layer_s_total.items(), key=lambda kv: kv[0])
        per_layer_str = " ".join(
            f"layer{lid}={(dt_s * 1000.0 / max(1, args.runs)):.3f}ms" for lid, dt_s in per_layer_items
        )
        print(f"[Time] _get_ngram_hashes per-layer: {per_layer_str}")
